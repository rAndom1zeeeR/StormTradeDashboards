-- [STORM] Daily Losing Traders Table
WITH
	...
	storm_update_position AS (
		...
		WHERE
			DATE_TRUNC ('day', FROM_UNIXTIME (tx_now)) >= CURRENT_DATE - INTERVAL '{{days}}' day
	),
	...
	-- Calculate PnL by trader to identify losing traders
	trader_pnl AS (
		SELECT
			trader_addr,
	...
	-- Final query - show only losing traders (negative PnL)
SELECT
	CONCAT (
...
FROM
	...
	JOIN trader_pnl pnl ON events.trader_addr = pnl.trader_addr
WHERE
	pnl.total_pnl > 0 -- Filter only traders with positive PnL (profitable traders)
GROUP BY
	events.trader_addr,
	tx.tx_count,
	vol.total_volume,
	...
	td.last_trade
ORDER BY
	SUM(events.pnl) DESC -- Order by PnL descending to show biggest winners first